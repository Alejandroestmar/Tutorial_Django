...
from .utils import parse_rfc3339_re, rfc3339_re
...
def loads(s, filename="<string>", translate=lambda t, x, v: v, object_pairs_hook=dict):
    ...
    def process_value(v, object_pairs_hook):
        ...
        if kind == "array":
            ...
            value = [
                process_value(item, object_pairs_hook=object_pairs_hook)
                for item in value
            ]
        elif kind == "table":
            value = object_pairs_hook(
                [
                    (k, process_value(value[k], object_pairs_hook=object_pairs_hook))
                    for k in value
                ]
            )
